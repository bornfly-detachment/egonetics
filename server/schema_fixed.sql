...
-- 消息表：存储过滤后的消息和思考内容（包含完整执行链）
CREATE TABLE IF NOT EXISTS messages (
    id TEXT PRIMARY KEY,                     -- 消息ID (OpenClaw message ID)
    session_id TEXT NOT NULL,                -- 所属会话ID
    message_type TEXT NOT NULL,              -- 消息类型: 'thinking' | 'message' | 'system' | 'tool_call' | 'tool_result'
    ...
    is_collapsible BOOLEAN DEFAULT 0,        -- 是否可折叠（tool_call/tool_result默认折叠）
...
CREATE TABLE IF NOT EXISTS thinking_annotations (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    message_id TEXT NOT NULL,                -- 对应的消息ID
    thinking_index INTEGER DEFAULT 0,        -- 消息中第几个thinking部分（从0开始）
...
CREATE INDEX IF NOT EXISTS idx_messages_is_collapsible ON messages(is_collapsible);
...
CREATE INDEX IF NOT EXISTS idx_thinking_annotations_message_id ON thinking_annotations(message_id);
