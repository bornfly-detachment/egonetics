...
app.get('/api/stats', (req, res) => {
  ...
  const queries = [
    ...
    { key: 'total_messages', sql: 'SELECT COUNT(*) as count FROM messages' },
    { key: 'total_thoughts', sql: 'SELECT COUNT(*) as count FROM messages WHERE message_type = "thinking"' },
    { key: 'total_conversations', sql: 'SELECT COUNT(*) as count FROM messages WHERE message_type = "message"' },
    { key: 'avg_quality', sql: 'SELECT AVG(quality_score) as avg FROM messages WHERE quality_score > 0' },
    { key: 'messages_by_role', sql: 'SELECT role, COUNT(*) as count FROM messages GROUP BY role' },
    { key: 'categories_count', sql: 'SELECT level, COUNT(*) as count FROM categories GROUP BY level' }
  ];
  ...
  queries.forEach(query => {
    db.get(query.sql, (err, row) => {
      if (err) {
        console.error(`❌ 查询统计 ${query.key} 失败:`, err.message);
      } else {
        results[query.key] = row;
      }
      ...
      if (completed === queries.length) {
        ...
        const formattedStats = {
          total_sessions: results.total_sessions?.count || 0,
          total_messages: results.total_messages?.count || 0,
          total_thoughts: results.total_thoughts?.count || 0,
          total_conversations: results.total_conversations?.count || 0,
          avg_quality: results.avg_quality?.avg ? parseFloat(results.avg_quality.avg.toFixed(2)) : 0,
          messages_by_role: results.messages_by_role || {},
          categories_by_level: results.categories_count || {}
        };
...
app.post('/api/messages/:id/annotation', (req, res) => {
  ...
  db.get('SELECT id FROM thinking_annotations WHERE message_id = ?', [messageId], (err, existing) => {
    if (err) { res.status(500).json({ error: err.message }); return; }
    
    if (existing) {
      db.run('UPDATE thinking_annotations SET suggested_revision = ?, updated_at = CURRENT_TIMESTAMP WHERE message_id = ?',
        [suggested_revision || '', messageId], function(err) {
          if (err) { res.status(500).json({ error: err.message }); return; }
          res.json({ success: true, message: '标注已更新' });
        });
    } else {
      db.run('INSERT INTO thinking_annotations (message_id, suggested_revision) VALUES (?, ?)',
        [messageId, suggested_revision || ''], function(err) {
          if (err) { res.status(500).json({ error: err.message }); return; }
          res.json({ success: true, message: '标注已创建' });
        });
    }
  });
});
...
app.get('/api/messages/:id/annotation', (req, res) => {
  const messageId = req.params.id;
  db.get('SELECT ta.*, m.content as original_content FROM thinking_annotations ta JOIN messages m ON ta.message_id = m.id WHERE ta.message_id = ?',
    [messageId], (err, row) => {
      if (err) { res.status(500).json({ error: err.message }); return; }
      if (!row) { res.json({ exists: false }); return; }
      res.json({ exists: true, annotation: row });
    });
});
...
app.post('/api/tags', (req, res) => {
  ...
  if (!name) { res.status(400).json({ error: '标签名称不能为空' }); return; }
  
  db.run('INSERT INTO categories (level, name, parent_id, description, is_predefined) VALUES (3, ?, ?, ?, 0)',
    [name, parent_id || null, description || ''], function(err) {
      if (err) { if (err.message.includes('UNIQUE')) { res.status(400).json({ error: '标签已存在' }); return; }
...
app.get('/api/tags', (req, res) => {
  db.all('SELECT * FROM categories ORDER BY level, name', (err, rows) => {
    if (err) { res.status(500).json({ error: err.message }); return; }
    res.json({ tags: rows.map(t => ({ ...t, is_predefined: t.is_predefined === 1 })) });
  });
});
...
app.post('/api/messages/:id/tags', (req, res) => {
    ...
    if (tag_names && tag_names.length > 0) {
      for (const name of tag_names) {
        await new Promise((resolve) => {
          db.run('INSERT OR IGNORE INTO categories (level, name, is_predefined) VALUES (3, ?, 0)', [name], function(err) {
            if (!err && this.lastID) createdTagIds.push(this.lastID);
            db.get('SELECT id FROM categories WHERE name = ? AND level = 3', [name], (err, row) => {
              if (row && !createdTagIds.includes(row.id)) createdTagIds.push(row.id);
              resolve();
            });
          });
        });
  ...
  processTags().then(createdIds => {
    const allTagIds = [...(tag_ids || []), ...createdIds];
    if (allTagIds.length === 0) { res.json({ success: true }); return; }
    let completed = 0;
    allTagIds.forEach(tagId => {
      db.run('INSERT OR IGNORE INTO message_categories (message_id, category_id) VALUES (?, ?)', [messageId, tagId], () => {
        if (++completed === allTagIds.length) res.json({ success: true });
      });
    });
  });
});
...
// 获取消息的所有标签
app.get('/api/messages/:id/tags', (req, res) => {
  const messageId = req.params.id;
  db.all('SELECT c.* FROM categories c JOIN message_categories mc ON c.id = mc.category_id WHERE mc.message_id = ?',
    [messageId], (err, rows) => {
      if (err) { res.status(500).json({ error: err.message }); return; }
      res.json({ tags: rows.map(t => ({ ...t, is_predefined: t.is_predefined === 1 })) });
    });
