...
router.post('/canvases/:id/nodes', (req, res) => {
  const { entity_type, entity_id, x = 100, y = 100, expanded_level = 0 } = req.body;
  ...
  pagesDb.run(
    `INSERT OR IGNORE INTO canvas_nodes (id, canvas_id, entity_type, entity_id, x, y, expanded_level, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [id, req.params.id, entity_type, entity_id, x, y, expanded_level, ts],
    function(err) {
      if (err) return res.status(500).json({ error: err.message });
      if (this.changes === 0) {
      ...
      res.status(201).json({ id, canvas_id: req.params.id, entity_type, entity_id, x, y, expanded_level, created_at: ts });
    }
  );
});
