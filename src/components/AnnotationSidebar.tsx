            ...
            <div className="pl-6 space-y-1">
              ...
              {usage.inputTokens !== undefined && (
                <div className="text-neutral-300">
                  💰 {language === 'zh' ? '输入' : 'In'}: {usage.inputTokens} /{' '}
                  {language === 'zh' ? '输出' : 'Out'}: {usage.outputTokens || 0}
                </div>
              )}
              {usage.durationMs !== undefined && (
                <div className="text-neutral-300">⏱️ {formatDuration(usage.durationMs)}</div>
              )}
            </div>
          </div>
        )}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {tags.map((tag) => (
              <span
                key={tag}
                className="flex items-center space-x-1 px-2 py-1 bg-purple-900/30 text-purple-300 rounded-lg text-sm"
              >
                <span>{tag}</span>
