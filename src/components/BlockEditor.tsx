...
function BlockNode({
  ...
  const handleKD = (e: React.KeyboardEvent<HTMLElement>) => {
    if (e.key === 'Enter' && !e.shiftKey && block.type !== 'code') { onAddAfter(block.id); setEditingId(null) }
    else if (e.key === 'Tab') { e.preventDefault(); e.shiftKey ? onOutdent(block.id) : onIndent(block.id) }
    else if (e.key === 'Backspace' && getPlainText(block.content.rich_text) === '') { e.preventDefault(); onDelete(block.id) }
  ...
  return (
    <div className={`relative ${isDragging ? 'opacity-25' : ''}`} onMouseLeave={() => setDropZone(null)}>
      <div
        ...
        onClick={() => { if (!isSubpage) { setFocusedId(block.id); setEditingId(block.id) } }}
      >
        {/* 折叠 + 拖拽 */}
      ...
      {childrenVisible && children.map((child, idx) => (
        <BlockNode key={child.id} block={child} level={level + 1} allBlocks={allBlocks} getChildren={getChildren}
          ...
          siblingIndex={idx} onNavigate={onNavigate} onCreateSubpage={onCreateSubpage} />
      ))}
    </div>
  )
}
        ...
        <div onClick={e => e.stopPropagation()}>
          {rootBlocks.map((block, idx) => (
            <BlockNode key={block.id} block={block} level={0} allBlocks={blocksMap} getChildren={getChildren}
              ...
              onNavigate={onNavigate} onCreateSubpage={handleCreateSubpage} />
          ))}
        </div>
        <div className="h-32 cursor-text" onClick={e => { e.stopPropagation(); const r = rootBlocks; if (r.length) addAfter(r[r.length-1].id) }} />
