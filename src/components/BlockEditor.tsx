...
function SubpageBlockView({
  ...
  return (
    ...
    >
      ...
      <ChevronRight
        size={13}
        className="ml-auto text-neutral-600 group-hover:text-neutral-400 shrink-0"
      />
    </button>
  )
}
...
function ImageBlock({
  ...
  return (
    <div
      ref={containerRef}
      className="relative my-1 group/imgblock"
...
function BlockViewContent({
  ...
  if (type === 'subpage') {
    return (
      <SubpageBlockView block={block} onNavigate={onNavigate} onCreateSubpage={onCreateSubpage} />
    )
  }
                          ...
                          onBlur={(e) =>
                            onTableCommit?.(
                              rows.map((r, rIdx) =>
                                rIdx === ri
                                  ? r.map((c, cIdx) =>
                                      cIdx === ci ? { rich_text: makeSegs(e.target.value) } : c
                                    )
                                  : r
                              )
                            )
            ...
            {[
              {
                ...
                action: () =>
                  onTableCommit?.([
                    ...rows,
                    Array.from({ length: cols }, () => ({ rich_text: [] as RichTextSegment[] })),
                  ]),
              },
              {
                label: '+ 列',
                action: () =>
                  onTableCommit?.(rows.map((r) => [...r, { rich_text: [] as RichTextSegment[] }])),
              },
              ...(rows.length > 1
                ? [{ label: '- 末行', action: () => onTableCommit?.(rows.slice(0, -1)) }]
                : []),
              ...(cols > 1
                ? [
                    {
                      label: '- 末列',
                      action: () => onTableCommit?.(rows.map((r) => r.slice(0, -1))),
                    },
                  ]
                : []),
            ].map((btn) => (
  ...
  if (type === 'math')
    return (
      <code className="font-mono text-yellow-300 bg-neutral-800 px-2 py-0.5 rounded text-sm">
        {getPlainText(segs) || <span className="text-neutral-500 italic">LaTeX…</span>}
      </code>
    )
    ...
    return (
      <div className="bg-neutral-800/60 rounded-lg p-5 text-center my-1 border border-neutral-700/40">
        <code className="font-mono text-yellow-300 text-base">
          {getPlainText(segs) || <span className="text-neutral-500 italic">输入 LaTeX 公式…</span>}
        </code>
      </div>
    )
  ...
  if (type === 'image') {
    ...
    return url ? (
      <ImageBlock
        ...
        onWidthChange={onUpdateContent ? (w) => onUpdateContent({ imageWidth: w }) : undefined}
      />
    ) : (
    ...
    return url ? (
      <div className="my-1 flex items-center gap-3 px-4 py-3 bg-neutral-800/60 rounded-lg border border-neutral-700">
        ...
        <div className="flex-1 min-w-0">
          {content.fileName && (
            <p className="text-xs text-neutral-500 mb-1 truncate">{content.fileName}</p>
          )}
  ...
  if (type === 'file') {
    ...
    if (ext === 'pdf')
      return (
        <div
          className="my-1 rounded-lg overflow-hidden border border-neutral-700 bg-neutral-900"
          style={{ height: '600px' }}
        >
    ...
    return (
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="flex items-center gap-2 my-1 px-4 py-2.5 bg-neutral-800/60 rounded-lg border border-neutral-700 hover:bg-neutral-800 text-sm text-blue-400"
      >
  ...
  if (type === 'toc') {
    ...
    const indent: Record<string, string> = {
      heading1: 'ml-0 font-medium',
      heading2: 'ml-4',
      heading3: 'ml-8 text-neutral-400',
      heading4: 'ml-12 text-neutral-500',
    }
    return (
      <div className="bg-neutral-800/30 rounded-lg p-4 my-1 text-sm border border-neutral-700/30">
        <div className="text-[10px] text-neutral-600 font-semibold uppercase tracking-widest mb-2">
          目录
        </div>
        <div className="space-y-1">
          {headings.map((h) => (
            <div
              key={h.id}
              className={`${indent[h.type] ?? ''} text-blue-400 hover:text-blue-300 cursor-pointer truncate`}
            >
    ...
    return (
      <div className="grid gap-3 my-1" style={{ gridTemplateColumns: `repeat(${n}, 1fr)` }}>
        {Array.from({ length: n }).map((_, i) => (
          <div
            key={i}
            className="min-h-10 rounded border border-dashed border-neutral-700/50 p-2 text-xs text-neutral-600"
          >
            第 {i + 1} 列
          </div>
        ))}
  ...
  switch (type) {
    case 'heading1':
      return (
        <h1 className="text-[32px] font-bold text-neutral-100 mt-2 leading-tight">
          <RichText segments={segs} />
        </h1>
      )
    case 'heading2':
    ...
    case 'numbered':
      return (
        <div className="flex items-baseline gap-2 text-neutral-200">
          <span className="text-neutral-500 shrink-0 tabular-nums text-sm min-w-[1.5rem] text-right">
            {content.numberStart ?? 1}.
          </span>
          <span>
            <RichText segments={segs} />
          </span>
...
function InlineEditor({
  ...
  if (block.type === 'divider' || block.type === 'toc') {
    commit()
    return <BlockViewContent block={block} allBlocks={allBlocks} />
  }
      ...
      return (
        <div className="flex items-center gap-2 w-full">
          <input
            ...
            className={`${shared} text-sm flex-1`}
            placeholder={ph}
          />
  ...
  return (
    <textarea
      ...
      onInput={autoResize}
      rows={1}
      className={`${shared} overflow-hidden leading-relaxed`}
