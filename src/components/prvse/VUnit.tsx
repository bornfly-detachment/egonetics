...
function CheckRow({ item }: { item: ChecklistItem }) {
  return (
    <div className="flex items-start gap-2 py-1">
      <span className="text-[10px] shrink-0 mt-0.5">{CHECK_ICON[item.result]}</span>
      <div className="min-w-0">
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] font-mono text-white/40 w-14 shrink-0">{item.type}</span>
...
function MetricBar({ metric }: { metric: Metric }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-[9px] font-mono text-white/35 w-20 shrink-0 truncate">{metric.label || metric.type}</span>
...
function RewardRow({ reward }: { reward: RewardFunction }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-[9px] font-mono text-white/35 w-24 shrink-0 truncate">{reward.label || reward.type}</span>
      <div className="flex-1 h-1 rounded-full bg-white/[0.06] overflow-hidden">
        <div className="h-full rounded-full" style={{ width: `${pct}%`, background: color }} />
      </div>
      <span className="text-[9px] font-mono w-8 text-right shrink-0" style={{ color }}>{pct}%</span>
    </div>
  )
}
...
export default function VUnit({
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-[11px] font-black font-mono shrink-0" style={{ color: '#fb923c' }}>V</span>

        {/* 裁决级别 */}
        <span className="text-[10px] font-mono shrink-0 text-white/40">
        ...
        {/* 通过率 */}
        <span className="text-[9px] font-mono shrink-0" style={{ color: vs.color }}>
          {passCount}/{totalCount}
        </span>

        {/* 裁决 badge */}
        <span
          className="flex items-center gap-1 text-[9px] font-mono px-1.5 py-0.5 rounded border shrink-0"
