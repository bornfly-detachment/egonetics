...
function EdgeRow({ edge }: { edge: RelationEdge }) {
  ...
  return (
    <div className="flex items-center gap-1.5 py-1 text-[10px] font-mono">
      <span className="text-white/40 w-14 truncate shrink-0">{edge.sourceNode}</span>
      <span style={{ color: lc.primary }} title={edge.edgeType}>
        {EDGE_TYPE_SYMBOLS[edge.edgeType]}
      </span>
      <span className="text-white/40 w-14 truncate shrink-0">{edge.targetNode}</span>
...
export default function RUnit({
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-[11px] font-black font-mono shrink-0" style={{ color: '#a78bfa' }}>R</span>
        <span className="text-[10px] font-mono shrink-0" style={{ color: lc.primary }}>{lc.label}</span>
